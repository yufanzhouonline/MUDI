...
class scRNAseq(RNAseq):
    ...
    def loadClusterDEG(self, degPath = '/data/scRNAseq/clusterDEG.txt'):
        self.clusterDEG = pd.read_csv(degPath, header=0, sep='\t')
        self.clusterDEG['label'] = ['D' + str(i+1) for i in self.clusterDEG.cluster]
    
    def loadVarGene(self, varGenePath = '/data/vargene.txt'):
    ...
    def loadClusterN(self, clusterPath = '/data/scRNAseq/clusterNumber.txt'):
        self.clusterNumber = pd.read_csv(clusterPath, header=None, sep='\t')
    ...
    def loadcellType(self, cellTypePath = '/data/scRNAseq/clusterCellType.txt'):
        '''
